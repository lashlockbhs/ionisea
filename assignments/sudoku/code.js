...
const multiplier = 100 //this should not exceed a few million or things WILL break (also breaks with negatives)
const mode = 'avg' //'sci', 'power', 'trunc', 'avg', 'commas', 'page.Crash()' (this will not do anything)
...
const checkIfAnomaly = (acc)=>{
  if (acc.length-1 >= (attempts.toString().length)) {
    ...
    if (acc.length-2 >= (attempts.toString().length)){ 
      drawText('jackpot with ' + (mode == 'sci' ? sciNote(acc) : mode == 'power' ? acc.length-1 : mode == 'trunc' ? truncate(acc) : mode == 'avg' ? averageResults(logs.array) : mode == 'commas' ? commas(acc): acc) + '!', Math.random()*width - height, Math.random()*height, 'blue', height/6)
      logs.jackpots++
      return 'blue'
    }
      return 'green'
