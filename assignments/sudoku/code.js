...
registerOnclick((x,y) => {
  ...
  drawText(mode == 'sci' ? sciNote(acc) : mode == 'power' ? acc.length-1 :mode == 'commas'? commasNote(acc) : acc, x, y, acc.length-1 >= attempts.toString().length  || acc.length - 1 < attempts.toString().length -4? 'green' : 'black',25)
});