...
if (winner[1] == 'h'){
  drawLine(width/2 - height/3,height/6+height*(winner[2]/3), width/2 + height/3, height/6+height*(winner[2]/3), 'black', 15)
} else if (winner[1] = 'v'){
...
registerOnclick((x, y) => {
 if (y < 1/3 * height){
  ...
  if (coordArray[yPos][xPos] == '' && winSearch(player) == undefined){
    ...
    winSearch(player) == player ? drawText(player + ' wins!', width/6, height*300/480,player == player1 ? '#2596be' : '#C62828', height/2) : 0;
