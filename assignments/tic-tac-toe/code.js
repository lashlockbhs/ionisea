...
if (winner.winType == 'h'){
 ...
 drawLine(width/2-height/(winner.winLoc == 1 ? 2 : -2), height * winner.winLoc, width/2-height/(winner.winLoc == 1 ? -2 : 2), 'gray', 8)
}
}
let winner = {}
const winSearch = (player) =>{
  for(let c = 0; c<=2; c++){
    ...
    if ((coordArray[0][0] == player) && (coordArray[1][1] == player) && (coordArray[2][2] == player)) winner = {winner: player,winType: 'h' ,winLoc: 1};
    if ((coordArray[2][0] == player) && (coordArray[1][1] == player) && (coordArray[0][2] == player)) winner = {winner: player,winType: 'h' ,winLoc: 2};
  }
  if (winner != undefined){
  gameWin(winner)
